...
int main(int argc, char *argv[])
{
   ...
   FiniteElementSpace *R_space = new FiniteElementSpace(mesh, dg_coll);
   ...
   double sigma = -1.0;
   double kappa = (order + 1) * (order + 1) * 2.0 * nu;
   mVarf->AddDomainIntegrator(new VectorDiffusionIntegrator);
   mVarf->AddInteriorFaceIntegrator(new DGElasticityIntegrator(zero, k, sigma, kappa));
   mVarf->Assemble();
