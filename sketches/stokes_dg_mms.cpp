...
int main(int argc, char *argv[])
{
   ...
   BilinearForm *gVarf(new BilinearForm(pfes));
   gVarf->AddDomainIntegrator(new DiffusionIntegrator);
   gVarf->AddInteriorFaceIntegrator(new DGDiffusionIntegrator(one, sigma, kappa));
   gVarf->Assemble();
   gVarf->Finalize();
...
// {
//    printf("BE GetDim\n");
//    for (int k = 0; k < pfes->GetNBE(); k++)
//    {
//       const FiniteElement *be = pfes->GetBE(k);
//       printf("BE %d GetDim: %d\n", k, be->GetDim());
//    }
// }

   pform->Assemble();
   gVarf->AddMult(g_gf, *pform);
   ...
   // pVarf->FormLinearSystem(empty, p, *pform, AP, P1, G1);

   int maxIter(10000);
   double rtol(1.e-15);
   double atol(1.e-15);
   CGSolver solver2;
   solver2.SetOperator(*pVarf);
   ...
   if (!pres_dbc)
   {
      printf("Setting OrthoSolver\n");
      ortho.SetSolver(solver2);
      ortho.SetOperator(*pVarf);
   }
