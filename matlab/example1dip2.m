...
Al = - [2 -1;
        -1 0];
Al(1,1) = Al(1,1) + gammai;
Al = Al / dxi; Al = sparse(Al);
Kblocks{1}(1:2,1:2) = Kblocks{1}(1:2,1:2) + Al;

bl = - [1; -1] + gammai * [1; 0]; bl = bl / dxi;
bunits(1:2,1) = bunits(1:2,1) + bl * bdri(1);

Ar = - [0 -1;
        -1 2];
Ar(end,end) = Ar(end,end) + gammai;
Ar = Ar / dxi; Ar = sparse(Ar);
Kblocks{end}(end-1:end,end-1:end) = Kblocks{end}(end-1:end,end-1:end) + Ar;

br = - [-1; 1] + gammai * [0; 1]; br = br / dxi;
bunits(end-1:end,end) = bunits(end-1:end,end) + br * bdri(2);
