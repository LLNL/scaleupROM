...
Nsub = 4;
Nx = Ne / Nsub + 1;
% number of interface constraints
Ninter = 2 * (Nsub - 1); % one for continuity, one for smoothness.
...
Kglobal = sparse([Kk A;A' zeros(Ninter, Ninter)]);
bglobal = [bk; zeros(Ninter,1)];

uglobal = Kglobal \ bglobal;
uglobal = reshape(uglobal(1:end-Ninter),[Nx,Nsub]);
figure(2)
plot(xg, uglobal,'o-');
...
for k = 1:Nsample
    ...
    uglobal = Kglobal \ bglobal;
    us(:,:,k) = reshape(uglobal(1:end-Ninter),[Nx,Nsub]);
end
...
Kglobal = sparse([Kk A;A' zeros(Ninter)]);
bglobal = [bk; zeros(Ninter,1)];

fomU = Kglobal \ bglobal;
fomU = reshape(fomU(1:end-Ninter), [Nx, Nsub]);

% LSPG projection
KR = sparse([Kk * P A; A' * P zeros(Ninter,Ninter)]);
w = KR \ bglobal;
romU = reshape(P * w(1:end-Ninter), [Nx, Nsub]);
...
Ai = [0 1 -1 0;
      -1 1 1 -1]';
Nx = size(bunits,1);
A = zeros(Nx*numBlocks,2*(numBlocks-1));
for k = 1:numBlocks-1
    A(k*Nx-1:k*Nx+2,2*k-1:2*k) = Ai;
end
A = A / dxi;
